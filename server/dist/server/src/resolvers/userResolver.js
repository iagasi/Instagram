"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.userTypeDefs = exports.userResolvers = void 0;
var userService_1 = require("../services/userService");
function ModifyMessage(sender) {
    return function (target, propertyKey) {
        // use  sender, target and propertyKey arguments ...
    };
}
exports.userResolvers = {
    Query: {
        findUser: function (parrent, args) { return __awaiter(void 0, void 0, void 0, function () {
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        console.log("find user");
                        return [4 /*yield*/, userService_1.UserService.getSingleUser(args.id)];
                    case 1: return [2 /*return*/, _a.sent()];
                }
            });
        }); },
        findByNameSurname: function (parrent, args) { return __awaiter(void 0, void 0, void 0, function () {
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4 /*yield*/, userService_1.UserService.findPersonsByNameAndSurname(args.name)];
                    case 1: return [2 /*return*/, _a.sent()];
                }
            });
        }); },
        getUserPrefferences: function (parrent, args) { return __awaiter(void 0, void 0, void 0, function () {
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        console.log("prefferences");
                        return [4 /*yield*/, userService_1.UserService.userPrefferences(args.id)];
                    case 1: return [2 /*return*/, _a.sent()];
                }
            });
        }); },
        getUserData: function (parrent, args) {
            return __awaiter(this, void 0, void 0, function () {
                return __generator(this, function (_a) {
                    switch (_a.label) {
                        case 0: return [4 /*yield*/, userService_1.UserService.getUserData(args.id)];
                        case 1: return [2 /*return*/, _a.sent()];
                    }
                });
            });
        },
        getUserFriends: function (parrent, args) {
            return __awaiter(this, void 0, void 0, function () {
                return __generator(this, function (_a) {
                    switch (_a.label) {
                        case 0: return [4 /*yield*/, userService_1.UserService.getUserFriends(args.id)];
                        case 1: return [2 /*return*/, _a.sent()];
                    }
                });
            });
        },
    },
    Mutation: {
        subscribeTo: function (parrent, args) {
            return __awaiter(this, void 0, void 0, function () {
                return __generator(this, function (_a) {
                    switch (_a.label) {
                        case 0: return [4 /*yield*/, userService_1.UserService.subscribe(args.input.myId, args.input.candidateId)];
                        case 1: return [2 /*return*/, _a.sent()];
                    }
                });
            });
        },
        deleteFollower: function (parrent, args) {
            return __awaiter(this, void 0, void 0, function () {
                return __generator(this, function (_a) {
                    switch (_a.label) {
                        case 0: return [4 /*yield*/, userService_1.UserService.deleteFollower(args.input.myId, args.input.candidateId)];
                        case 1: 
                        //return null;
                        return [2 /*return*/, _a.sent()];
                    }
                });
            });
        },
        deleteFollowing: function (parrent, args) {
            return __awaiter(this, void 0, void 0, function () {
                return __generator(this, function (_a) {
                    switch (_a.label) {
                        case 0: return [4 /*yield*/, userService_1.UserService.deleteFollowing(args.input.myId, args.input.candidateId)];
                        case 1: return [2 /*return*/, _a.sent()];
                    }
                });
            });
        },
        changeNameSurname: function (paretn, args) {
            return __awaiter(this, void 0, void 0, function () {
                return __generator(this, function (_a) {
                    switch (_a.label) {
                        case 0: return [4 /*yield*/, userService_1.UserService.changeNameSurname(args.input.myId, args.input.name, args.input.surname)];
                        case 1: return [2 /*return*/, _a.sent()];
                    }
                });
            });
        },
    },
};
exports.userTypeDefs = "\ntype User{\n _id:String\n  name: String\n  surname: String\n  image:String\n  email:String\n  password:String\n}\n\ntype UserPrefferencesType {\n  _id: String\n  userId:String\n  followers:[String]\n  followings:[String]\n  posts: [String]\n  saved: [String]\n  tagged: [String]\n}\n\ntype PrefferencesType{\n  user:User!\n  prefferences:UserPrefferencesType\n}\ntype UserFriendsType{\n  followers:[User]\n  followings:[User]\n}\ntype LoginType{\n  _id:String\n  acessToken:String\n  \n}\ntype RefreshType{\n  acessToken:String,\n  refreshToken:String\n}\ntype Query{\n  findUser(id:String):User\n  getUserFriends(id:String):UserFriendsType\n  getUserPrefferences(id:String):[UserPrefferencesType]\n  getUserData(id:String):PrefferencesType\n  findByNameSurname(name:String):[User]\n}\n\n\ninput FriendsHandlerType{\nmyId:String\ncandidateId:String\n\n}\n\ninput changeNameSurnameType {\n    myId: String\n    name: String\n    surname: String\n}\n\ntype Mutation{\n  subscribeTo(input:FriendsHandlerType):PrefferencesType\n  deleteFollower(input:FriendsHandlerType):PrefferencesType\n  deleteFollowing(input:FriendsHandlerType):PrefferencesType\n  changeNameSurname(input:changeNameSurnameType):User\n}\n";
